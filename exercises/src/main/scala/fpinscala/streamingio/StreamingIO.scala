  ...
  object Process {
    ...
    def count[I]: Process[I,Int] = {
      def go(cnt: Int): Process[I, Int] =
        Await {
          case Some(_) => Emit(cnt+1, go(cnt+1))
          case None => Halt()
        }
      go(0)
    }
    ...
    def mean: Process[Double,Double] = {
      def go(total: Double, cnt:Double): Process[Double, Double] =
        Await {
          case Some(d) => Emit((total+d)/(cnt+1), go(total+d,cnt+1))
          case None => Halt()
        }
      go(0.0, 0.0)
    }
    ...
    def sum2: Process[Double,Double] = loop(0.0)(
      (d, total) => ((total+d), (total+d))
    )

    def count3[I]: Process[I,Int] = loop(0)(
      (d, total) => (total+1, total+1)
    )
    ...
    def zip[A,B,C](p1: Process[A,B], p2: Process[A,C]): Process[A,(B,C)] =
      (p1, p2) match {
        ...
        case (Await(recv), _) => Await(
          (oa: Option[A]) => zip(recv(oa), feed(oa)(p2))
        )
        case (_, Await(recv)) => Await(
          (oa: Option[A]) => zip(feed(oa)(p1), recv(oa))
        )
      }
    ...
    def mean2: Process[Double, Double] =
      zip(sum, count[Double]) |> lift { case (s, c) => s / c}
     ...
     */
    def toCelsiusFile(f: java.io.File) = processFile(f, toCelsiusP, List[String]())(_ ++ List(_))

    def toCelsiusP: Process[String, String] =
      filter[String](line => !line.startsWith("#") || line.trim.length > 0) |>
        lift { x: String => toCelsius(x.toDouble).toString }
