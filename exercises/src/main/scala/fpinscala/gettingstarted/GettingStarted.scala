...
object PolymorphicFunctions {
  ...
  def isSorted[A](as: Array[A], gt: (A,A) => Boolean): Boolean = {
    @annotation.tailrec
    def go(low: Int, high: Int, gt: (A,A) => Boolean): Boolean = {
      if (low >= high) true
      else if (gt(as(low), as(low+1))) false
      else go(low+1, high, gt)
    }
    go(0, as.length - 1, gt)
  }
  ...
  */
  ...
  def main(args: Array[String]): Unit = {
    ...
    assert(isSorted(Array(5,1,2,3,4), (x:Int, y:Int) => x > y) == false)
    assert(partial1(3, (a: Int, b: Int) => a + b)(4) == 7)
    assert(curry((a: Int, b: Int) => a + b)(3)(4) == 7)
    assert(uncurry(curry((a: Int, b: Int) => a + b))(3, 4) == 7)
    assert(compose((a: Int) => a + 3, (b: Int) => b + 4)(1) == 8)
  }
}
